...
pub(crate) fn generate_digits() -> String {
    ...
    format!(
        "{semver} (ripgrep {} rev {})",
        crate::upstream::RIPGREP_VERSION,
        crate::upstream::RIPGREP_COMMIT,
    )
}
...
pub(crate) fn generate_long() -> String {
    ...
    writeln!(
        out,
        "ripgrep-upstream:{}",
        crate::upstream::RIPGREP_REPOSITORY
    )
    .unwrap();
    if !compile.is_empty() {
...
pub(crate) fn generate_pcre2() -> (String, bool) {
    ...
    {
        writeln!(out, "PCRE2 is not available in this build of elgrep.").unwrap();
        (out, false)
    }
}
