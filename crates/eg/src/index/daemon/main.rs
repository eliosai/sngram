...
const POSTINGS_MANIFEST: &str = "postings-v8/manifest.bin";
const POSTINGS_JSON_MANIFEST: &str = "postings-v8/manifest.json";
...
",
    ...
    fn child_consolidation_requires_clean_live_parent() {
        ...
        fs::create_dir_all(parent_state.join("index/postings-v8")).expect("parent index");
        fs::create_dir_all(child_state.join("index")).expect("child index");
        fs::write(parent_state.join("index/postings-v8/manifest.json"), "{}").expect("manifest");
