...
def test_capped_short_exhaustion_does_not_mint_final(tmp_path: Path):
    glob = write_fixture(tmp_path / "short", "short", ["x" * 1000] * 10, files=1)
    family = Family(
        id="short",
        weight=200_000,
        cap_bytes=200_000,
        sources=(Source("short", "local", "content", data_files=glob, cap_bytes=200_000),),
    )
    trainer = Trainer(
        families=[family],
        mint_dir=tmp_path / "bins",
    ...
    assert trainer.durable_bytes() == 10_000
    assert not (tmp_path / "bins" / "final_weights.bin").exists()
    assert "final" not in trainer.state.mints_done
    assert any(e.get("stage") == "exhausted_short" for e in events)
